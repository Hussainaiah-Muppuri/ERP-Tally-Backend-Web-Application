...
public class StockItemController {
	...
	public ResponseEntity<ResponseEntity<StockItemEntity>> update(@PathVariable Long stockItemId,
			@RequestBody StockItemEntity updateItem) {
		ResponseEntity<StockItemEntity> result = stockService.update(stockItemId, updateItem);
		return ResponseEntity.ok(result);
	}
	...
	public ResponseEntity<ResponseEntity<String>> deleteById(@PathVariable Long stockItemId) {
		ResponseEntity<String> result = stockService.deleteById(stockItemId);
		return ResponseEntity.ok(result);
	}

	@GetMapping("/getByUserIdAndCompanyId/{userId}/{companyId}")
	public ResponseEntity<List<StockItemEntity>> getByUserIdAndCompanyId(@PathVariable Long userId,
			@PathVariable Long companyId) {
		List<StockItemEntity> result = stockService.getByUserIdAndCompanyId(userId, companyId);
		return ResponseEntity.ok(result);
	}
}