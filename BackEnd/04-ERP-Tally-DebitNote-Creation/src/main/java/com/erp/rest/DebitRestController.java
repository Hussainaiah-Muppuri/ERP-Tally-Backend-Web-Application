...
public class DebitRestController {
    ...
    public ResponseEntity<List<DebitEntity>> getAllByUserIdAndCompanyId(@PathVariable Long userId,
                                                                        @PathVariable Long companyId) {
        List<DebitEntity> debitEntities = Services.getAllByUserIdAndCompanyId(userId, companyId);

        if (!debitEntities.isEmpty()) {
            return new ResponseEntity<>(debitEntities, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
    }
