...
public class UnitController {
	...
	@PostMapping("/saveAll")
	public ResponseEntity<?> saveAllUnits(@RequestBody List<UnitEntity> units) {
	    List<UnitEntity> savedUnits = unitService.saveAllUnits(units);
	    if (!savedUnits.isEmpty()) {
	        return new ResponseEntity<>("Units saved successfully", HttpStatus.CREATED);
	    } else {
	        return new ResponseEntity<>("Failed to save units", HttpStatus.INTERNAL_SERVER_ERROR);
	    }
	}
