...
export default function UpdateSales() {
  ...
  const { companyId, userId, saleNo } = useParams();
  ...
  const fetchSalesOrderDetails = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:9093/api/sales/get/${saleNo}`);
  ...
  const updateSalesOrder = async () => {
    ...
    if (isConfirmed) {
      ...
      try {
        if (saleNo) {
          const response = await axios.put(`http://localhost:9093/api/sales/update/${saleNo}`, data, {
            headers: {
          ...
          history.push(`SalesList/${userId}/${companyId}`);
  ...
  const saveSalesOrder = async () => {
    ...
    if (isConfirmed) {
      ...
      try {
        const response = await axios.post(' http://localhost:9093/api/sales/save', data, {
          headers: {
            'Content-Type': 'application/json',
          },
        });
  ...
  return (
    <div className='container soform'>
      ...
      <div className="d-flex justify-content-between mb-3">
        ...
        <h4 style={{ textAlign: 'center' }}>Sales Voucher Update</h4>
        <Link to={`/SalesList/${userId}/${companyId}`} className="btn btn-primary">
          Sales Entries
        </Link>
      </div>
      <div className="Sales-Order-Div">
        ...
        <div className="mainSale">
          ...
          <form onSubmit={saleNo ? updateSalesOrder : saveSalesOrder}>
            <div className="SalesInput">
              <label>
                <div className="high">Sales </div>
              </label>
