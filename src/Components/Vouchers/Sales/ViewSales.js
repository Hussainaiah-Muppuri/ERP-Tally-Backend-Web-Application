...
export default function ViewSales() {
  ...
  const { saleNo, userId, companyId } = useParams();
  ...
  useEffect(() => {
    axios.get(`http://localhost:9093/api/sales/get/${saleNo}`)
      .then(response => {
  ...
  return (
    <div className='container'>
      ...
      <div className="d-flex justify-content-between mb-3">
        ...
        <h3 style={{ textAlign: 'center' }}>sales Voucher Details</h3>
        ...
        <Container maxWidth="md" className="view-journal-card-container">
          <Card elevation={8} className="view-journal-card" >
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} md={7}>
                <Typography variant="body1" className="journal-info">
                  <strong>Sales No:</strong> {sales.saleNo}
                </Typography>
                <Typography variant="body1" className="journal-info">
            ...
            <div className="view-journal-button-container">
              <Link to={`/SalesList/${userId}/${companyId}`} className="back-button">
