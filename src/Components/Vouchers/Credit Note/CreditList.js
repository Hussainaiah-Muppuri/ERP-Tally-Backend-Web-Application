...
export default function CreditList() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:9094/api/creditNote/getAllBy/${userId}/${companyId}`)
      .then(response => {
  ...
  const handleUpdateSalesOrderVoucher = (creditNo) => {
    ...
    if (isConfirmed) {
      ...
      history.push(`/UpdateCredit/${userId}/${companyId}/${creditNo}`);
    }
  };

  const handleDeleteSalesOrderVoucher = async (creditNo) => {
    // Ask for confirmation
    ...
    if (isConfirmed) {
      try {
        ...
        await axios.delete(`http://localhost:9094/api/creditNote/delete/${creditNo}`);

        // Update the list of journal vouchers after deletion
        const updatedSalesOrderVouchers = salesOrderVouchers.filter(voucher => voucher.creditNo !== creditNo);
  ...
  return (
    <div className='container'>
      ...
      <div className="d-flex justify-content-between mb-3">
        ...
        <h4 style={{ textAlign: 'center' }}>Credit Note Entries</h4>
        <Link to={`/CreditNote/${userId}/${companyId}`} className="btn btn-primary">
          Create Credit Note
        </Link>
      </div>
        ...
        <table className="table table-info table-borderless text-center" style={{ fontSize: "medium" }}>
          ...
          <tbody>
            {salesOrderVouchers.map((voucher, index) => (
              <tr key={voucher.creditNo} className='action-buttons'>
                ...
                <td className='form-rows'>
                  <button
                    onClick={() => handleViewSalesOrderVoucher(voucher.creditNo)}
                    className="btn btn-primary mr-2"
                  >
                    View
                  </button>
                  <button onClick={() => handleUpdateSalesOrderVoucher(voucher.creditNo)} className="btn btn-warning mr-2">
                    Update
                  </button>
                  <button onClick={() => handleDeleteSalesOrderVoucher(voucher.creditNo)} className="btn btn-danger">
                    Delete
                  </button>
