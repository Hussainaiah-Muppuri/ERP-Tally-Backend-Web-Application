...
export default function UpdateCredit() {
  ...
  const { companyId, userId, creditNo } = useParams();
  ...
  const fetchSalesOrderDetails = async () => {
    ...
    try {
      ...
      const response = await axios.get(` http://localhost:9094/api/creditNote/get/${creditNo}`);
  ...
  const updateSalesOrder = async () => {
    ...
    if (isConfirmed) {
      ...
      try {
        if (creditNo) {
          const response = await axios.put(`http://localhost:9094/api/creditNote/update/${creditNo}`, data, {
            headers: {
  ...
  return (
    <div className='container soform'>
      ...
      <div className="d-flex justify-content-between mb-3">
        ...
        <h4 style={{ textAlign: 'center' }}>Credit Note Update</h4>
      ...
      <div className="Sales-Order-Div">
        ...
        <div className="mainSale">
          ...
          <form onSubmit={creditNo ? updateSalesOrder : saveSalesOrder}>
